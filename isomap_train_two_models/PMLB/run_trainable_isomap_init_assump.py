...
def get_data(df_name, save_path=None):
    ...
    return X_train, X_test, X_validation, np.expand_dims(y_train, axis=1), np.expand_dims(y_test, axis=1), np.expand_dims(y_validation, axis=1)
...
def run_isomap(run_number=5):
    ...
    for ds_name in os.listdir('regression'):
        ds_path = f'regression/{ds_name}/isomap_linear_nn_assumption'
        if not os.path.exists(ds_path):
            os.makedirs(ds_path)
        X_train, X_test, X_validation, y_train, y_test, y_validation = get_data(ds_name,
                                                                                save_path=f'regression/{ds_name}')

        # TRAIN INIT ASSUMPTION #
        '''task_model = nn.Sequential(nn.Linear(X_train.shape[-1], 512, dtype=float32),
        ...
        features = torch.tensor(X_train, dtype=float32).to(device)
        ...
        for ep in range(150):
            ...
            # print(f'Task model: epoch {ep}/{150}, loss={task_loss.item()}')
            task_losses.append(task_loss.item())
            task_loss.backward()
            task_optim.step()

        prediction = task_model(features).cpu().detach().numpy()'''
        #init_assump = torch.tensor(pairwise_distances(prediction, prediction, metric='l1'), dtype=float32)
        init_assump = torch.tensor(pairwise_distances(y_train, y_train, metric='l1'), dtype=float32)
        ...
        #dist_train_new = torch.tensor(pairwise_distances(X_train, X_train[pts]), dtype=float32).to(device) / max_val
        ...
        for r in range(run_number):
            ...
            for epoch in range(isomap_epochs):
                ...
                isomap_loss = isomap_criterion(output.to(torch.float32),
                                               train_target.reshape_as(output).to(torch.float32))
