...
class Evolution:
    ...
    def plot_evolution_fitnesses(self, reverse: bool = False, save_path: str = None):
        ylab = 'Fitness'
        for k in self.evolution_history.keys():
            generation = self.evolution_history[k]
            gen_fitnesses = [g['fitness'] for g in generation]
            if reverse:
                gen_fitnesses = 1 / np.array(gen_fitnesses)
                ylab = 'Loss'
            plt.scatter([k]*len(gen_fitnesses), gen_fitnesses, c='b')
    ...
    def run(self, multicriteria=False):
        evolution_history = {}
        ...
        for i in range(self.iterations):
            ...
            print('Update graph coordinates')
            pop_operators.update_population_coordinates()
    ...
    @property
    def best_individ(self):
        """
        Property that return best individ in current population individs pool by its fitness value
        """
        fitnesses = [ind.fitness for ind in self.population.individs_pool]
        best_index = np.argmax(fitnesses)
        best_individ = self.population.individs_pool[best_index]
        return best_individ

    def best_individ_model(self):
        """
        Function for training model with best graph individ
        """
        individ_model = deepcopy(self.base_model)
        individ_model.train(self.best_individ)
        return individ_model
