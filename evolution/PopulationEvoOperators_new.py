...
from evolution.IndividEvoOperators_new import IndividEvoOperators
...
class PopulationEvoOperators:
    ...
    def filter_population(self, size_to_save):
        ...
        valid_indices = [i for i, ind in enumerate(self.population.individs_pool)
                         if abs(ind.eigenvalues[0] - ind.eigenvalues[1]) >= 0.1]
...
class PopulationMultiEvoOperators(PopulationEvoOperators):
    ...
    def scalar_product(self, vector: np.ndarray, criteria: np.ndarray):
        ...
        :param criteria: individ's criteria
        """
    ...
    def fast_non_dominated_sorting(self):
        ...
        # Number of elements which dominate on i-candidate
        domination_count = np.zeros(len(population))
        # Number of elements on which i-candidate dominate
