...
class PopulationEvoOperators:
    ...
    def fiter_population(self):
        ...
        self.population.individs_pool = [x for _, x in sorted(zip(fitnesses, self.population.individs_pool), key=lambda pair: pair[0])]

        self.population.individs_pool = self.population.individs_pool[-self.population.size+len(elite):]
