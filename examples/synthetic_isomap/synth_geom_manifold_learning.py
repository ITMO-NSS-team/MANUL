...
from examples.data.synthetic_data_generation import geometries, noisy_manifold
...
def process_geometry(geometry_name, n_samples=1000, n_basis_points=200,
    ...
    working_folder = os.path.join('examples', 'synthetic_isomap', 'results', geometry_name)
    ...
    y = colors  # Use colors as regression target
    ...
    fps_path = os.path.join(working_folder, 'fps_indices.npy')
    if not os.path.exists(fps_path):
        print(f"\nApplying Farthest Point Sampling to select {n_basis_points} basis points...")
        fps_indices = memory_efficient_fps(X_train, n_basis_points, batch_size=500)
        np.save(fps_path, fps_indices)
        print(f"  Sampled {len(fps_indices)} key points from training data")
    else:
        print(f"\nLoading existing FPS indices...")
        fps_indices = np.load(fps_path)
        print(f"  Loaded {len(fps_indices)} basis points")

    X_train_sparse = X_train[fps_indices]
    y_train_sparse = y_train[fps_indices]


    print(f"\nTraining GradientIsomap (latent_dim={latent_dim}, epochs={epochs})...")
    ...
    # Get trained distance matrix and base projections
    ...
    print("\nComputing projections for all training data...")
    projector = Projector(
        source_data=X_train,
    ...
    print(f"  Train projections computed: {train_projections.shape}")

    print("\nComputing projections for validation data...")
    ...
    np.save(os.path.join(working_folder, 'val_projections.npy'), val_projections)
