...
log_folder = 'results/1000_20000_2024_05_22-08_33_15_AM'
...
def form_mean_table():
    ...
    for i in range(b.shape[0]):
        if errors_df['pvals'][i] > 0.05 or np.isnan(errors_df['pvals'][i]):
            ...
            if test_errors_df[i, 2] == np.max(test_errors_df[i]):
                b[i, :] = 2
            else:
                b[i, :] = 0
    ...
    plt.title('OpenML multiclass datasets\n50-20000 instances\nF1 score ')
    plt.tight_layout()
    plt.savefig(f'{log_folder}/mult_openml_50-20000.png', dpi=300)
    plt.show()
