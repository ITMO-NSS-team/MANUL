...
from examples.gradient_isomap.mnist_regression.baseline import baseline_train_test
from examples.gradient_isomap.mnist_regression.manifold_learning import mnist_manifold_learning
from examples.gradient_isomap.mnist_regression.manifold_regularization import manifold_regularization
...
if __name__ == "__main__":
    ...
    for n in range(n_runs):
        ...
        # Baseline and regularized models share the same architecture and operate
        # on raw 784-pixel features - the manifold only supplies a regularization
        # signal during training, it is not used as an input projection.
        model_architecture = [nn.Linear(784, 1000, dtype=torch.float64),
                              nn.ReLU(),
        ...
        print(f'Run baseline model on MNIST...')
        ...
        print(f'Run regularization on MNIST...')
    ...
    plt.suptitle('MNIST - Baseline vs Regularized Comparison',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    comparison_path = os.path.join(outputs_dir, 'mnist_comparison_boxplots.png')
    plt.savefig(comparison_path, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"✓ Comparison plots saved to: {comparison_path}")
    combined_metrics_path = os.path.join(outputs_dir, 'mnist_combined_metrics.csv')
