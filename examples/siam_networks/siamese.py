...
class DISTANCE_DATASET(Dataset):
    def __init__(self, root, train, index_i, download=False):
        super(DISTANCE_DATASET, self).__init__()
    ...
    def __getitem__(self, index):
        image_1 = self.data[self.index_i].clone().float()

        image_2 = self.data[index].clone().float()
...
def siamese_dist_exp(exp_num, model):
    i_min = exp_num * 1000
    i_max = i_min + 1000

    MNIST_weights_train = torch.zeros((i_max - i_min, 60000)).half()
    for index_i in range(i_min, i_max):

        MNIST_distance_dataset_train = DISTANCE_DATASET('../Data', train=True, download=False, index_i=index_i)

        MNIST_Train_batch_size = 10000

        MNIST_distance_loader_train = torch.utils.data.DataLoader(MNIST_distance_dataset_train,
                                                                  batch_size=MNIST_Train_batch_size, shuffle=False)

        output_list = []

        with torch.no_grad():
            for images_1, images_2 in MNIST_distance_loader_train:
                images_1, images_2 = images_1.to(device), images_2.to(device)
                outputs = model(images_1, images_2).squeeze().half()
                output_list.append(outputs)
            outputs = torch.cat(output_list)
            MNIST_weights_train[index_i % 1000] = outputs

        print("Processed={} of {} ({:.2f} %)".format(index_i % 1000, i_max - i_min, (index_i % 1000) / (i_max - i_min)))

    print('Saving data from {} to {}'.format(i_min, i_max))
    w_train = np.array(MNIST_weights_train)
    np.save("siamese_MNIST_train_set_{}_{}.npy".format(i_min, i_max), w_train)
...
if __name__ == '__main__':
    ...
    siamese_state_dict = torch.load("siamese_network.pt")

    model.load_state_dict(siamese_state_dict)

    model.eval()

    siamese_dist_exp(int(sys.argv[1]), model)

    #w_train=np.array(MNIST_weights_train)

    #np.save("siamese_MNIST_train_set.npy", w_train)

    #plt.imshow(w_train,vmax=0.1)
    #plt.colorbar()
    #plt.show()
