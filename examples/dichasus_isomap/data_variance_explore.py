...
from examples.dichasus_isomap.dichasus_data_process import load_data
...
H = load_data("dichasus-ad01.tfrecords")
H_flat = H.reshape(64, 1, 1024, -1)
real_part = np.real(H_flat)
imag_part = np.imag(H_flat)
H_processed = np.stack([real_part, imag_part], axis=-1)
H_processed = H_processed.reshape(H_processed.shape[0] *
                                  H_processed.shape[1] *
                                  H_processed.shape[2] *
                                  2,
                                  H_processed.shape[3]).T
...
analyser.plot_dimension_histograms(dataset_name="dichasus")
_, _ = analyser.plot_variance_threshold_analysis(H_processed,
                                                 dataset_name="dichasus")'''
