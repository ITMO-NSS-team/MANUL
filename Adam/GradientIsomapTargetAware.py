...
from Adam.IsomapNNTargetAware import IsomapNNTargetAware
...
class GradientIsomapTargetAware:
    def __init__(self, train_feature: torch.Tensor,
        ...
        # --- Target-aware info logging ---
        if use_target_modification:
            print(f"[TargetAware] Mode: {modification_mode}, weight={target_weight}, eps={epsilon}")
            print(f"[TargetAware] Modification applied ")
        else:
            print(f"[TargetAware] Disabled (baseline)")
    ...
    def train(self, use_init_assumption=False):
        ...
        if self.use_target_modification:
            # IsomapNNTargetAware - modification applied every epoch
            isomap_model = IsomapNNTargetAware(
                weights_initial_assumption=dist_train,
                n_components=self.latent_len,
                n_neighbors=self.n_neighbors,
                eigval_choice='MDS',
                # Target-aware parameters:
                targets=self.targets,
                use_target_modification=True,
                target_weight=self.target_weight,
                epsilon=self.epsilon,
                modification_mode=self.modification_mode
            )
        else:
            # Original IsomapNN for baseline
