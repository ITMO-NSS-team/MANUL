...
class GradientIsomapSASRec:
    ...
    def __init__(
        ...
        self.n_batches_inner = max(1, n_users_with_seq // batch_size)
                ...
                # Внешний градиент dL/dD_input в GradientIsomapCF считается по ВСЕМУ
        # тренировочному набору (preds_all = ncf_model(self.users_all, ...)).
        # Полный forward одним тензором здесь невозможен по памяти, поэтому
        # эквивалент — аккумуляция dL/dZ по полному проходу батчами.
        # n_outer_batches=None => n_batches_inner (каждый пользователь ровно раз).
        if self.n_outer_batches is None:
            self.n_outer_batches = self.n_batches_inner
        self.n_outer_batches = max(1, int(self.n_outer_batches))
        print(f"Outer grad accumulation: {self.n_outer_batches} batches/step "
              f"(inner epoch = {self.n_batches_inner} batches, "
              f"~{self.n_outer_batches * batch_size} sequences)")
    ...
    def train(self, val_loader=None, top_k: int = 10, device=None):
        ...
        for epoch in range(self.epochs):
            ...
            grad_item_Z_sum = torch.zeros_like(item_Z_full)
            outer_loss_sum = 0.0
            n_outer_used = 0

            for _ in range(self.n_outer_batches):
                seq, pos, neg = next(batch_iter)
                seq = seq.to(self.device)
                pos = pos.to(self.device)
                neg = neg.to(self.device)
                ...
                pos_logits, neg_logits = sasrec(
                    seq, pos, neg, item_Z_full
                )

                pos_valid = pos_logits[indices]
                neg_valid = neg_logits[indices]

                b_loss = (
                    loss_fn(pos_valid, torch.ones_like(pos_valid))
                    + loss_fn(neg_valid, torch.zeros_like(neg_valid))
                )

                grad_item_Z_batch = torch.autograd.grad(
                    outputs=b_loss,
                    inputs=item_Z_full,
                    retain_graph=False,
                    create_graph=False,
                ...
                grad_item_Z_sum.add_(grad_item_Z_batch.detach())
                outer_loss_sum += float(b_loss.detach().item())
                n_outer_used += 1

            if n_outer_used == 0:
                raise RuntimeError(
                    "Outer step did not contain any non-padding targets."
                )

            avg_train_loss = outer_loss_sum / n_outer_used
            grad_item_Z_mean = grad_item_Z_sum / n_outer_used

            # Передаем усредненный task-gradient в IsomapNN.
            item_Z_full.backward(grad_item_Z_mean)
            ...
            if had_bad_grad:
                print(
                    f"  [Guard] Outer epoch {epoch}: "
                    f"{grad_nonfinite_count} non-finite gradient entries "
                    f"zeroed before step()",
                    flush=True,
                )
            ...
            if val_loader is not None:
                ...
                with torch.no_grad():
                    item_Z_val_raw = isomap_model().to(torch.float32)
                    item_Z_val = pad_item_Z(item_Z_val_raw)
                    ...
                    for v_seq, v_cands, v_labels in val_loader:
                        ...
                        v_logits = sasrec.predict_candidates(
                            v_seq,
                            v_cands,
                            item_Z_val,
                        )

                        v_labels = v_labels.to(dtype=v_logits.dtype)
                        val_batch_loss = loss_fn(v_logits, v_labels)

                        total_val_loss_outer += val_batch_loss.item()
                        n_val_batches_outer += 1

                avg_val_loss = (
                    total_val_loss_outer / max(1, n_val_batches_outer)
                )
